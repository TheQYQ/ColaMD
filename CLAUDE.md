...
ColaMD is a WYSIWYG markdown editor built on Electron + Vue 3. It supports CommonMark, GitHub Flavored Markdown, math (KaTeX), Mermaid diagrams, PlantUML, and multiple editing modes (focus, typewriter, source-code).

- **Version**: see `package.json`
- **License**: MIT
- **Repository**: https://github.com/TheQYQ/ColaMD
...
<repo-root>/
  package.json              Workspace orchestrator — every CI-facing script
                            ...
                            colamd ...`. CI invocations are unchanged.
  ...
  eslint.config.js          Root ESLint v9 flat config (covers desktop;
                            packages/muya self-lints via its own antfu
  ...
  packages/
    desktop/                The Electron app (name: "colamd").
    ...
    muya/                   The editor engine
                            ...
                            spec suites (CommonMark + GFM). Consumed by the
                            desktop renderer as its editor engine. See
...
desktop-specific script to `packages/desktop` via `pnpm --filter colamd`,
...
- No semicolons
- Single quotes
- TypeScript with `strict: true`; see `packages/desktop/tsconfig.base.json`
...
workspace package (`@muyajs/core`) that the renderer and tests consume
...
- `packages/muya/CLAUDE.md` — muya engine layout, conventions, and commands
- `docs/` — long-form developer docs
...
- **Workspace deps**: muya's own npm runtime deps (`github-markdown-css`, `katex`, `dompurify`, `snabbdom`, …) are declared in `packages/muya/package.json` so module resolution stays inside the workspace rather than walking out to a parent directory.
