...
| 多余导出       | `knip --workspace packages/desktop`：全量跑为 **48 未使用导出 + 46 未引用类型**（`develop` 重跑）                                                                                                                                                                                       | 实测三分（见 O20）：**49 处仅 export 多余**（已清 `cab7cb8`）、**29 处去 export 后连本文件都不引用＝真死代码**（移交 O13）、**1 处 `FileSearcher` 与 quick-open 分支冲突需排除**。首轮曾把死代码误判为"只是关键字多余"，判据已改为"去 export 后 eslint 是否立刻报 unused-vars" |
