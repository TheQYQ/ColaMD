...
| Store                                                                | 文件                                                                      | 职责                                                                  |
| -------------------------------------------------------------------- | ------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `editor`                                                             | `store/editor.ts`（**1977 行**，约 85 个 action；注册型动作已不含业务体） | 标签、当前文件、TOC、保存/导出/关闭/切换/自动存 —— 事实上的上帝 store |
...
| 版本历史                 | 主进程 `main/versionHistory/` + `store/editor.ts:1911,1936`（`SAVE_VERSION_SNAPSHOT` / `LISTEN_FOR_VERSION_RESTORE`）                                                                                                                                                                                                                                                                                           | ⚠️ 无面板，侧栏 tab 已在 `f4f4a13` 下线，监听器不可达 |
...
| 非空断言       | `pnpm check` = 0 errors / **99 warnings**（94 `no-non-null-assertion` + 5 `no-unused-vars`；2026-09-22 重测。此前是 103 = 98 + 5，少的 4 条是目录监听 reducer 外提后不再需要的 `projectTree.value!`），99 条按目录：`src/main` 54 / `test/e2e` 23 / `test/unit` 12 / `src/renderer` 10                                                                                                                                                                        | `main/windows/editor.ts` 原占 44 处（31%），已 `86b8711` 清零（见路线图 O25）；现存最多的是 `main/menu/actions/paragraph.ts` 13 处与 `main/menu/actions/file.ts` 8 处，同为"窗口必定已建"的模式                                                                                |
...
| 上帝文件       | `store/editor.ts` 1,973（O12 六步 + 共享保存请求那一刀）、`editor.vue` 1,831、`prefComponents/image/…/uploader/index.vue` 1,186（2026-09-22 重测）；引擎 `block/base/format.ts` 1,851、`muya.ts` 1,733                                                                                                                                                                                                                                                        | 已有 O12 覆盖（桌面包）。`muya/src/config/emojis.ts` 12,983 行是数据文件，不计入                                                                                                                                                                                               |
