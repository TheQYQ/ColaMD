...
>
> 内容全部以当前工作区代码为准（分支 `develop`，`05e108d`，2026-09-19）。路径与行为如与本文冲突，以代码为准并请回修本文。
...
| 版本历史                 | 主进程 `main/versionHistory/` + `store/editor.ts:875,900`（`SAVE_VERSION_SNAPSHOT` / `LISTEN_FOR_VERSION_RESTORE`）                                                                                                                                                                                                                                                                                             | ⚠️ 无面板，侧栏 tab 已在 `d08ed6a` 下线，监听器不可达 |
