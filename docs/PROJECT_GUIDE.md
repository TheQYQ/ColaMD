...
| 版本历史                 | 主进程 `main/versionHistory/` + `store/editor.ts:875,900`（`SAVE_VERSION_SNAPSHOT` / `LISTEN_FOR_VERSION_RESTORE`）                                                                                                                                                                                                                                                                                             | ⚠️ 无面板，侧栏 tab 已在 `f4f4a13` 下线，监听器不可达 |
...
| 非空断言       | `pnpm check` = 0 errors / **95 warnings**（94 `no-non-null-assertion` + 1 `no-unused-vars`；2026-09-26 重测。基线从 99 降到 95：删掉的 4 条 `no-unused-vars` 都是真死代码——`main/versionHistory/index.ts` 里没人用的 `SnapshotLabel` 表（调用方各传字面量）、`util/pdf.ts` 的 `catch (_)`（换成可选 catch 绑定）、`test/e2e/xss.spec.ts` 取了从不读的 `page`、`util/themeRegistry.ts` 的死 import；留下的唯一一条是 `codeMirror/index.ts` 的 `getModeFromName`，它要牵出约 40 行连带删除与一个失去读者的 `./modes` 模块，另立 O13 项处理。2026-09-22 重测。此前是 103 = 98 + 5，少的 4 条是目录监听 reducer 外提后不再需要的 `projectTree.value!`），99 条按目录：`src/main` 54 / `test/e2e` 23 / `test/unit` 12 / `src/renderer` 10 | `main/windows/editor.ts` 原占 44 处（31%），已 `86b8711` 清零（见路线图 O25）；现存最多的是 `main/menu/actions/paragraph.ts` 13 处与 `main/menu/actions/file.ts` 8 处，同为"窗口必定已建"的模式                                                                                |
