...
| 多余导出       | `knip --workspace packages/desktop`：**47 个未被其他文件 import 的导出** + **48 个未被引用的导出类型**                                                                                                                                                                          | 绝大多数是"export 关键字多余"（例：`contextMenu/*/menuItems.ts` 的 `NEW_FILE`/`CLOSE_THIS` 等在本文件内构建菜单时使用），**不是死代码**；见 O20                                            |
