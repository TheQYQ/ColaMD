...
- **第 6 步 part B 落点**：`editor.vue` 还剩 2169 行，可继续切的是（按体量）Muya 创建块与它那份 options 字面量（`onMounted` 内约 1900-2010 段）、滚动/光标定位一族（`getScrollContainer` / `scrollToCursor` / 锚点跟随）、图片插入与相对路径解析一族、以及拖拽与粘贴。切完这些再回到 `store/editor.ts` 的 action 簇与 `util/theme.ts:61`（全包 complexity 43 榜首，O21 阈值要再下调必须先拆它）。
