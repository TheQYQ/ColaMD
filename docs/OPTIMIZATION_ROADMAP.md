...
- **纠正上面这条清单里的一项**：阅读时长**不是**零覆盖。`editor-input.spec.ts:175-177` 已经点完四态循环并钉住 `ceil(words / 200)`（同一测试从 `editor-input.spec.ts:155` 的注释起就写明"fourth mode"）。所以 ④ 的真实缺口是 6 项，不是 7 项；那 6 项逐条核对后确认 `test/e2e` 下没有任何匹配选择器。
- **④ 已落地**（分支 `test/o14-e2e-anchors`，两个新 spec、6 项各有至少一条结构断言）：
  ...
  - `packages/desktop/test/e2e/reading-position-anchors.spec.ts`（2 例）钉大纲跟随高亮（滚动 → `editor.vue:1071 updateActiveTocEntry` → `bus 'toc-active-changed'` → `sideBar/toc.vue:119 setCurrentKey`，断言 `.el-tree-node.is-current` 唯一且标签文字随滚动换）与进入源码模式保阅读位置（`editor.vue:705` 的 `flush: 'sync'` Just-In-Time 计算 `muyaIndexCursor` → `sourceCode.vue:561` 选中并把外层容器滚到该行）。
  - **两处实测推翻了我先写下的断言**，记下以免第二次踩：① 行内注释的 `%%` 标记**留在 DOM 里**，muya 用 `font-size: 0` 隐藏（`muya/src/assets/styles/inlineSyntax.css:21`），`innerText` 并不剥掉它——所以"渲染文本里不该有 `%%`"是**假断言**，改成钉注释体本身（`toHaveText('aside')`）；② 大纲高亮只在容器 `scroll` 事件里重算（`editor.vue:1906`），文档从未滚动过时把 `scrollTop` 赋成同值不产生事件，"先看 Alpha"这条初态根本没有高亮——所以用例改成先向下滚、最后再回顶。顺带一条 Playwright 细节：`:text-is` 只匹配**包含该文字的最小元素**，所以要点 `span.mu-plain-text` 而不是它的父 `span.mu-paragraph-content`（后者计数恒为 0）。
  - **变异验证**（新增断言不是空转）：第一个 spec 的 4 处改动（alert 类型改 `caution`、去掉 `definitionList` 的推送、注释文本改 `'asideX'`、TOC 条目数改 3）→ 4 例全红，且定义列表那条的失败是 `Received: 0`，正好证明 `dl` 只可能来自偏好链路；第二个 spec 的 2 处改动（预期标签换成别的章节、去掉点段落那一步）→ 2 例全红，后者失败在 `filler line 0`，证明 CodeMirror 的光标确实由 WYSIWYG 光标带过去而不是恰好在第 0 行。还原后两个 spec `--repeat-each=2` 共 12 例全绿（14.4s），单次耗时 67-236ms。
  - 门禁：`pnpm check` 退出 **0**（103 problems / **0 errors**，与基线同数）、`vue-tsc --noEmit` 无输出；E2E 只跑了这两个新文件，全量 `pnpm test:e2e` 留到合入后。
...
| PR-14  | O14 CI 矩阵与覆盖率报告 —— **④ 已实现** `test/o14-e2e-anchors`（两个新 spec / 6 例，见 §3 O14"④ 已落地"）；①②③（build 腿前置单测、e2e mac 腿、非阻断覆盖率）仍是 YAML -only 待做                                                                                                      | 原写"依赖 PR-15"，实测两条补丁不影响任何现有腿的结论，不再互为前置；①②③ 本地无法验收，只能在 CI 腿上看 |
...
3. **E2E 缺口比自述更大。** 第五/六/七梯队三处"验证"段都写了"e2e（Playwright）未跑"——事实如此，且这些新能力（阅读时长、大纲跟随高亮、模式切换保滚动、GitHub Alerts、定义列表、行内注释、`[toc]` 块）在 63 个 desktop E2E spec 中**零覆盖**。已并入 O14④。
   - **本条的清单有一项是假阳性**：阅读时长在 `editor-input.spec.ts:175-177` 已有结构断言（四态循环 + `ceil(words / 200)`），当时 `grep` 按"阅读时长/min"找选择器没命中它。真实缺口是 6 项。
   - **④ 已按 6 项落地**（`test/o14-e2e-anchors`，见 §3 O14 的"④ 已落地"条），落地过程又推翻了两条我先写下的断言（`%%` 标记是否出现在 `innerText`、初态是否有大纲高亮），已在原处记全。
