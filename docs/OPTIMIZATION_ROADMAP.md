...
| 维度         | 实测值                                                                                                                                                                                                                                                                                         | 测量方法                                                                                                                              |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| 桌面端规模   | 244 个 ts/vue 文件、40,518 行（2026-09-22 O12 前五步后重测：新增 4 个模块、行数净 +177；合入 14 分支时是 240 / 40,341）                                                                                                                                                                        | `find packages/desktop/src … \→ wc -l`                                                                                                |
| 引擎规模     | 220 个 ts 文件（不含 `__tests__`）、48,485 行                                                                                                                                                                                                                                                  | 同上，`packages/muya/src`                                                                                                             |
| 最大文件     | `editor.vue` 2321（O12 ② 未开始）、`store/editor.ts` 1977（前五步从 2347 提出 370 行，第 5 步因具名动作各带签名与理由注释反增 12 行，见 §3 O12）、`prefComponents/image/…/uploader/index.vue` 1193、`commands/index.ts` 766、`menu/menus.ts` 724                                               | `wc -l`                                                                                                                               |
...
- **第 6 步落点**：剩下的可测性欠账集中在两处——`HANDLE_DISK_CHANGE` 的 reload 分支只测到"tab 被换成磁盘内容"，auto-save 计时器真正到点写盘仍只有 E2E 覆盖；以及 `editor.vue`（2321 行，O12 ② 的 composable 拆分：`useEditorLifecycle` / `useEditorPaste` / `useEditorModes`）还没动过。全包 complexity 榜首 `util/theme.ts:61`（43）若要再下调阈值，得先拆它。
...
| PR     | 内容                                                                                                                                                                                                        | 说明                                                                 |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| PR-13… | O12 上帝文件分解（store 提 services → 拆 composable，每步 E2E 全绿）—— **第 1-5 步已实现**（前四步 `0839766`/`88274b5`/`ed03def`/`83eb532`，第 5 步六个提交；store 2347 → 1977 行，注册型动作已不含业务体） | 每步可独立 revert，禁止与功能改动混提；下一刀见 §3 O12 "第 4 步落点" |
