...
| 维度         | 实测值                                                                                                                                                                                                                      | 测量方法                                                                                                                              |
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| 桌面端规模   | 243 个 ts/vue 文件、40,408 行（2026-09-21 O12 前三步后重测：新增 3 个模块、行数净 +67；合入 14 分支时是 240 / 40,341）                                                                                                      | `find packages/desktop/src … \→ wc -l`                                                                                                |
| 引擎规模     | 220 个 ts 文件（不含 `__tests__`）、48,485 行                                                                                                                                                                               | 同上，`packages/muya/src`                                                                                                             |
| 最大文件     | `store/editor.ts` 1976（O12 前三步已从 2347 提出 371 行，见 §3 O12）、`editor.vue` 2321、`prefComponents/image/…/uploader/index.vue` 1193、`commands/index.ts` 766、`menu/menus.ts` 724                                     | `wc -l`                                                                                                                               |
...
| 测试面       | 5 套共 434 个 spec 文件：desktop 单测 **73**（原 61，本轮新增 10 份 + O25 拆窗竞态 1 份 + O12(3) 标签规则 1 份）、desktop E2E 63、muya 单测 223、muya 一致性 4、muya E2E 71（2026-09-21 重测，五套逐套数过：72 + 63 + 298） | `find … -name '*.spec.ts' \→ wc -l`                                                                                                   |
...
- **第 4 步落点**：拆 `LISTEN_FOR_CONTENT_CHANGE`（complexity 44）与那 4 个百行级 action（`loadChange` 102、`LISTEN_FOR_FILE_CHANGE` 75、`LISTEN_FOR_SET_PATHNAME` 71、`LISTEN_FOR_BOOTSTRAP_WINDOW` 66）——从这一步起才允许下调 O21 的两条 warn 阈值。
...
| PR     | 内容                                                                                                                                                    | 说明                                                                 |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| PR-13… | O12 上帝文件分解（store 提 services → 拆 composable，每步 E2E 全绿）—— **第 1、2、3 步已实现**（`0839766`、`88274b5`、`ed03def`，store 2347 → 1976 行） | 每步可独立 revert，禁止与功能改动混提；下一刀见 §3 O12 "第 4 步落点" |
