...
- **第 3 步落点**（下一步就该碰真正的"大"了）：store 尾部只剩 `getRootFolderFromState`；剩下的 2008 行里主体是 action 本体，要按 O12 ① 的清单（保存 / 导出 / 标签生命周期 / 图片）把**成簇的 action**提成服务，并把 `LISTEN_FOR_CONTENT_CHANGE`（`store/editor.ts:1489`，complexity 44）拆开——那才是能下调 O21 阈值的步骤。
