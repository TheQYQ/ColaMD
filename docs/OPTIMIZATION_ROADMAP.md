...
| 维度         | 实测值                                                                                                                                                                                                 | 测量方法                                                                                                                              |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| 桌面端规模   | 241 个 ts/vue 文件、40,347 行（2026-09-21 O12 第 1 步后重测：新增 1 个 service、总行数 +6；上一次是合入 14 分支后的 240 / 40,341）                                                                     | `find packages/desktop/src … \→ wc -l`                                                                                                |
| 引擎规模     | 220 个 ts 文件（不含 `__tests__`）、48,485 行                                                                                                                                                          | 同上，`packages/muya/src`                                                                                                             |
| 最大文件     | `store/editor.ts` 2190（O12 第 1 步已从 2347 提出 157 行，见 §3 O12）、`editor.vue` 2321、`prefComponents/image/…/uploader/index.vue` 1193、`commands/index.ts` 766、`menu/menus.ts` 724               | `wc -l`                                                                                                                               |
...
- **这一步不动 O21 的阈值**：44（complexity）钉的是 `store/editor.ts:1506` 的 `LISTEN_FOR_CONTENT_CHANGE`、277（max-lines-per-function）钉的是 `store/project.ts:82` 的 setup（两条都用"把阈值下调一档再跑 eslint"的探针实测归因，O21 当年记的 `:1525` 已被本轮搬迁上移 19 行）。搬走的是整段函数、没有拆小任何一个，所以"每步下调一档"要等到真正拆到这两条函数体的步骤才成立，现在下调只会立刻造出新告警。
- 已量好的**第 2 步落点**（同样是"整段搬、不拆逻辑"）：store 尾部 `2019-2190` 共约 172 行的模块级纯函数——`getRootFolderFromState`、`adjustTrailingNewlines`/`trimTrailingNewlines`，以及会话缓冲序列化合集（`toSerializableValue` + `BufferedTabState`/`BufferedRestoreWarning`/`BufferedEditorState` 三个类型与三个 factory）；后者与既有的 `store/bufferedState.ts`（146 行）同属一个关注点。**顺带发现的一处重复**：`byteLengthUtf8` 在 `store/editor.ts:80` 与 `util/themeMarket.ts:219` 各有一份（后者是本地小实现），下一步可合并。
- 门禁（第 1 步，分支上实跑）：`pnpm check` 退出 **0**（105 warnings / 0 errors，与 O25 之后的基线逐项相同，未新增 unused-vars 说明搬过来的 import 全部用得上）、`pnpm test:unit` 退出 **0**（72 文件 / 907 通过 + 1 跳过）、`pnpm build` 退出 **0**。全量 `pnpm test:e2e` 退出 **1**：**4 failed / 223 passed**，与 O25 那一轮的失败集合**逐条同名**（`all-blocks-roundtrip` 四条字节稳定性，连 `test-results` 目录后缀 `40697/26f41/ecbfe/dd82f` 都一致），即 §4.1 记录的 Windows 既有红，通过数也完全相同 ⇒ 这一步对真窗口行为零影响。
