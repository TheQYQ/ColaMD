...
| 维度         | 实测值                                                                                                                                                                                                                                             | 测量方法                                                                                                                              |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| 桌面端规模   | 244 个 ts/vue 文件、40,435 行（2026-09-21 O12 前四步后重测：新增 4 个模块、行数净 +94；合入 14 分支时是 240 / 40,341）                                                                                                                             | `find packages/desktop/src … \→ wc -l`                                                                                                |
| 引擎规模     | 220 个 ts 文件（不含 `__tests__`）、48,485 行                                                                                                                                                                                                      | 同上，`packages/muya/src`                                                                                                             |
| 最大文件     | `store/editor.ts` 1965（O12 前四步已从 2347 提出 382 行，见 §3 O12）、`editor.vue` 2321、`prefComponents/image/…/uploader/index.vue` 1193、`commands/index.ts` 766、`menu/menus.ts` 724                                                            | `wc -l`                                                                                                                               |
...
| 测试面       | 5 套共 435 个 spec 文件：desktop 单测 **74**（原 61，本轮新增 10 份 + O25 拆窗竞态 1 份 + O12(3) 标签规则 1 份 + O12(4) 变更谓词 1 份）、desktop E2E 63、muya 单测 223、muya 一致性 4、muya E2E 71（2026-09-21 重测，五套逐套数过：72 + 63 + 298） | `find … -name '*.spec.ts' \→ wc -l`                                                                                                   |
...
- **下一步（第 5 步）落点**：阈值不再由 `LISTEN_FOR_CONTENT_CHANGE` 绑定，剩下的靶子是 `util/theme.ts:61`（43）与那 4 个百行级 action（`loadChange` 102、`LISTEN_FOR_FILE_CHANGE` 75、`LISTEN_FOR_SET_PATHNAME` 71、`LISTEN_FOR_BOOTSTRAP_WINDOW` 66）；`editor.vue` 2321 行的 composable 拆分（O12 ②）也还没开始。
