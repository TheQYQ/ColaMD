...
- **长度门为什么还不能下调**（本轮实测）：把 `max-lines-per-function` 临时压到 120 全量跑 eslint 读数，最大仍是 **`store/project.ts:82`（setup，277）**，第二名是本轮自己搬出来的 `useEngineOptionSync.ts:55`（202），再往下 `store/layout.ts:48`（133）。所以 277 这条阈值维持不动是对的——**下一刀想降长度门，只有 `store/project.ts` 的 setup 一个落点**，拆 `editor.vue` 已经不会再影响它。
