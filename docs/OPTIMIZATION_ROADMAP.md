...
| 维度         | 实测值                                                                                                                                                                                                 | 测量方法                                                                                                                              |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| 桌面端规模   | 242 个 ts/vue 文件、40,356 行（2026-09-21 O12 第 2 步后重测：两步共新增 2 个模块、行数净 +15；合入 14 分支时是 240 / 40,341）                                                                          | `find packages/desktop/src … \→ wc -l`                                                                                                |
| 引擎规模     | 220 个 ts 文件（不含 `__tests__`）、48,485 行                                                                                                                                                          | 同上，`packages/muya/src`                                                                                                             |
| 最大文件     | `store/editor.ts` 2008（O12 前两步已从 2347 提出 339 行，见 §3 O12）、`editor.vue` 2321、`prefComponents/image/…/uploader/index.vue` 1193、`commands/index.ts` 766、`menu/menus.ts` 724                | `wc -l`                                                                                                                               |
...
| 测试面       | 5 套共 433 个 spec 文件：desktop 单测 **72**（原 61，本轮新增 10 份 + O25 的拆窗竞态 1 份）、desktop E2E 63、muya 单测 223、muya 一致性 4、muya E2E 71（2026-09-21 重测，五套逐套数过：72 + 63 + 298） | `find … -name '*.spec.ts' \→ wc -l`                                                                                                   |
...
- **第 3 步落点**（下一步就该碰真正的"大"了）：store 尾部只剩 `getRootFolderFromState` 那 12 行。用脚本数过剩下的 2008 行：**70 个 action 共 1398 行**，按 O12 ① 的关注点分簇是——标签生命周期 18 个 action / 363 行（最大 `FORCE_CLOSE_TAB` 64、`CLOSE_TABS` 52）、保存线 11 个 / 202 行（最大 `HANDLE_AUTO_SAVE` 44）、导出 2 个 / 46 行、图片 4 个 / 67 行；另有 4 个百行级单点：`loadChange` 102、`LISTEN_FOR_FILE_CHANGE` 75、`LISTEN_FOR_SET_PATHNAME` 71、`LISTEN_FOR_BOOTSTRAP_WINDOW` 66。**先切标签簇**（边界最清楚：入口都是 `IFileState` + tabId，出口都是 `webContents.send` / bus），并把 `LISTEN_FOR_CONTENT_CHANGE`（`store/editor.ts:1489`，complexity 44）拆开——那才是能下调 O21 阈值的步骤。
...
| PR     | 内容                                                                                                                                      | 说明                                                                 |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| PR-13… | O12 上帝文件分解（store 提 services → 拆 composable，每步 E2E 全绿）—— **第 1、2 步已实现**（`0839766`、`88274b5`，store 2347 → 2008 行） | 每步可独立 revert，禁止与功能改动混提；下一刀见 §3 O12 "第 3 步落点" |
