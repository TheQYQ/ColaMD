...
- **part B 剩下的**：Muya 创建块与它那份 options 字面量（`onMounted` 内约 1900-2010 段）、图片插入与相对路径解析一族、以及拖拽与粘贴。切完这些再回到 `store/editor.ts` 的 action 簇与 `util/theme.ts:61`（全包 complexity 43 榜首，O21 阈值要再下调必须先拆它）。
