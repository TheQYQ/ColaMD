...
| 维度         | 实测值                                                                                                                                                                                                                                                                                         | 测量方法                                                                                                                              |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| 桌面端规模   | 245 个 ts/vue 文件、40,653 行（2026-09-22 O12 六步后重测：新增 5 个模块、行数净 +312；合入 14 分支时是 240 / 40,341）                                                                                                                                                                          | `find packages/desktop/src … \→ wc -l`                                                                                                |
| 引擎规模     | 220 个 ts 文件（不含 `__tests__`）、48,485 行                                                                                                                                                                                                                                                  | 同上，`packages/muya/src`                                                                                                             |
| 最大文件     | `editor.vue` 2169（O12 ② 第 6 步切出引擎偏好镜像后）、`store/editor.ts` 1977（前五步从 2347 提出 370 行，第 5 步因具名动作各带签名与理由注释反增 12 行，见 §3 O12）、`prefComponents/image/…/uploader/index.vue` 1193、`commands/index.ts` 766、`menu/menus.ts` 724                            | `wc -l`                                                                                                                               |
...
| 测试面       | 5 套共 438 个 spec 文件：desktop 单测 **77**（原 61，本轮新增 10 份 + O25 拆窗竞态 1 份 + O12(3) 标签规则 1 份 + O12(4) 变更谓词 1 份 + O12(5) 磁盘变更、保存回环、bootstrap 3 份）、desktop E2E 63、muya 单测 223、muya 一致性 4、muya E2E 71（2026-09-21 重测，五套逐套数过：72 + 63 + 298） | `find … -name '*.spec.ts' \→ wc -l`                                                                                                   |
...
- **part B 剩下的**：`onMounted` 里剩下的引擎**装配**（插件注册、`new Muya(...)`/`init()`、事件接线）。原来"拖拽与粘贴"这一项在组件里其实没有独立的函数族——它就是 options 里的四个回调（`clipboardFilePath` / `clipboardText` / `imageAction` / `getPathForFile`），随这一刀一起集中到了 `muyaOptions.ts` 末尾（`imageAction` 的实现仍在 `useEditorImages.ts`）。切完装配再回到 `store/editor.ts` 的 action 簇与 `util/theme.ts:61`（全包 complexity 43 榜首，O21 阈值要再下调必须先拆它）。
...
| PR-8      | O7② 读通道路径域 —— **实现到划下来的那条线** `security/picker-grants-and-read-scope`：选择器结果即授权、`mt::uploader::upload` 载荷归属（实测是"渲染端可指定主进程执行哪个程序"的信任边界洞，已闭）、`read-file`/`readdir`/`rg::start` 的目录收域 + 6 条单测与 1 条 E2E 负例 | PR-7。剩：五个布尔探测通道（决定不收域，理由写在 `fs.ts`）与 `mt::rg::start` 的载荷归属（O8 仅剩的一处豁免） |
