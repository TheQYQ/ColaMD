...
| 维度         | 实测值                                                                                                                                                                                                                                                                                                                                                                  | 测量方法                                                                                                                              |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| 桌面端规模   | 251 个 ts/vue 文件、40,500 行（2026-09-22 菜单形状一刀后重测：文件 +6、行数净 −153；O12 六步时是 245 / 40,653）                                                                                                                                                                                                                                                         | `find packages/desktop/src … \→ wc -l`                                                                                                |
| 引擎规模     | 220 个 ts 文件（不含 `__tests__`）、48,485 行                                                                                                                                                                                                                                                                                                                           | 同上，`packages/muya/src`                                                                                                             |
| 最大文件     | `store/editor.ts` 1973（共享保存请求那一刀从 1977 减 4 行；前五步从 2347 提出 370 行，见 §3 O12）、`editor.vue` 1831、`prefComponents/image/…/uploader/index.vue` 1186、`main/menu/actions/file.ts` 979、`main/app/index.ts` 895、`commands/index.ts` 766。**菜单模板已不在榜上**：`file/edit/paragraph/format` 由 259/181/206/131 降到 147/74/97/39（2026-09-22 重测） | `wc -l`                                                                                                                               |
...
| IPC 面       | 主进程 invoke 通道**全部走 `typedHandle`**（41 处调用点；唯一的裸 `ipcMain.handle` 就是垫片自己 `ipc/typedHandle.ts:23`）——`mt::rg::start` 的那处豁免随 `8c0d917` 消失；64 `ipcMain.on`、95 处 `webContents.send`；契约 41 invoke + 82 send + 2 sync + 69 内部（2026-09-20 计，`typedHandle` 一条 2026-09-22 重测）                                                     | `grep -c` 于 `src/main`，契约计数于 `src/shared/types/ipc.ts`                                                                         |
| 测试面       | 5 套共 445 个 spec 文件：desktop 单测 **82**（原 61，本轮增到 77 后再 +5：读通道收域、选择器授权、主题注入、目录监听 reducer、菜单模板形状）、desktop E2E **65**（+2：Markdown 扩展锚点、阅读位置锚点）、muya 单测 223、muya 一致性 4、muya E2E 71（desktop 两套 2026-09-22 重数；muya 三套仍为 2026-09-21 的数）                                                       | `find … -name '*.spec.ts' \→ wc -l`                                                                                                   |
...
| 7    | 测试/lint 仅 Linux                                 | ⚠️ 部分 | `test.yml` 已含 windows，`e2e.yml` 已加 mac 腿、`build.yml` 五条腿打包前各自跑本平台单测、`test.yml` 多了非阻断 coverage job（O14 ①②③，分支 `chore/ci-matrix-coverage`）——但**这三条腿的红绿本地判不了，要等首次 CI 运行**；`lint.yml` 与全部 `muya-*.yml` 仍只有 ubuntu → 见 O14                                                            |
...
- **一处自纠**：`88274b5` 的提交信息写的 "2009 -> 1846 行" 是错的（那是把两段删除数加总后凭记忆写的）；`wc -l` 实测本步是 **2190 → 2008**。以本条为准。
...
- **第 3 步 `ed03def`（分支 `refactor/store-tab-ops`）——与前两步性质不同：这一步改了 action 本体**。新建 `store/tabOps.ts`（84 行）装标签簇共享的 5 条规则，`store/editor.ts` **2008 → 1976 行**：
...
- 门禁（第 4 步，分支上实跑）：`pnpm check` 退出 **0**（103 warnings / 0 errors）、`pnpm test:unit` 退出 **0**（74 文件 / 934 通过 + 1 跳过）、`pnpm knip`（依赖，阻断）退出 **0**、`pnpm build` 退出 **0**；全量 `pnpm test:e2e` 是在 `83eb532` 这个已提交态上重跑的（不是复用第 3 步的结果），退出 **1**：**4 failed / 223 passed**，失败集合与前四轮**逐条同名**（`all-blocks-roundtrip` 四条 `40697/26f41/ecbfe/dd82f`）。这条击键热路径另有 E2E 锚点 `toc-collapse-state-3028.spec.ts`（改标题后保折叠态），其注释原先写的 `UPDATE_TOC` 已改准为 `refreshTocIfChanged`。
- **第 5 步（分支 `refactor/store-registration-actions`，5 个提交 `98f2c10`…`eb26258`）——主题：注册型 action 不再承载逻辑**。四个"名字带 LISTEN\_ 却把业务写在 IPC 回调里"的 action 拆成"注册 + 具名业务动作"，每刀一个可独立 revert 的提交：
  - `98f2c10`：三处 auto-save 计时器拆除（`has`→`get`→`if (timer)`→`delete`）合成 `clearAutoSaveTimer(id)`，其中一处原本无条件 `clearTimeout(timer)`，合并后语义不变。
  - `c74339f`：`LISTEN_FOR_FILE_CHANGE` 的回调体 → `HANDLE_DISK_CHANGE`（新 spec 6 例，含 #1861 那条"只有元数据变了要闭嘴"）。
  - `62b630c`：`LISTEN_FOR_SET_PATHNAME` 的三个匿名回调 → `SET_PATHNAME` / `MARK_TAB_SAVED` / `TAB_SAVE_FAILURE`（新 spec 8 例）；保存帧号提取合成 `historyFrameId`（**id 为 0 必须留住**，所以判据是 `!== undefined` 而不是真值）；顺手删掉 `if (tab)`——同一函数早先已在 `!tab` 时 return，这个判断是死码。
  - `ee2be51`：`LISTEN_FOR_BOOTSTRAP_WINDOW` → `APPLY_BOOTSTRAP_EDITOR` + 纯函数 `initialTabsToOpen`（welcome / 空白 / 种子列表三档，tab-ops 加 4 例）。参数只声明它读的三个字段，不吞整份 `BootstrapEditorConfig`；`markdownList` 保持必需，没新增原代码没有的兜底。
  - `eb26258`：`loadChange` 的重载撤销边界块 → `takeReloadBoundary`（4 例，含"下标越过栈时仍释放一格"这条它原本就有的行为）。
...
- **合入前 review（只读 agent 看整段五个提交）判"无 BLOCKER"，但揪出三条我自己没做到的**，全部按 `bd30226` 补掉：
...
- **第 6 步 part A `2a808d8` + `1714dcf`（分支 `refactor/editor-engine-option-sync`）——第一次动 `editor.vue`（O12 ②）**。抽出 `components/editorWithTabs/useEngineOptionSync.ts`（284 行），装 **28 个同形 watcher**："这条偏好变了、引擎实例在，就把值 `setOptions` 推进去"。watcher 逐字搬（只重排缩进），脚本按"忽略空白后逐块比对"证明 28 块与改动前全等、组件里一个不剩。`editor.vue` **2321 → 2169 行**。同目录已有 `syntheticHistory.ts` / `lazyMarkdownPipeline.ts`，新增文件沿用这个"组件私有模块"惯例，不另起目录。
- **留下的判据（两条，`1714dcf` 才写准）**：① 同一个变更还要**做别的事**的 watcher 留在组件（source-mode CodeMirror 重新起样式、DOM 行宽规则、菜单可用性、原生拼写检查开关）；② 没有 `editor.value` 存在性守卫的也留在组件。第二条是被机器逼出来的：review 指出 `spellcheckerNoUnderline` 是纯镜像、按①的说法不该留下，我照它搬过去后 `vue-tsc` 立刻报 **TS18047 'editor.value' is possibly 'null'** —— 因为 composable 里 `editor` 声明成 `Ref<Engine | null>`，比组件原本的 `type MuyaInstance = any`（`editor.vue:176`）严。要么改类型说谎、要么把无守卫那条留在原地静默吞掉挂载前的变更，选后者，并把注释里的规则改成真实的两条。
...
- `test/unit/specs/menu-template-golden.spec.ts`（分支第一个提交 `afdd434`）是这一刀的**证据**不是附带品：四个模板每一项的 label / id / type / role / accelerator / visible / 顺序，连同**点击实际调到哪个函数、带什么参数**，先抓取再钉住；抓取用的 keybindings 替身按真表行为对未注册 id 返回 `null`。钉住前做过变异：把 `heading1` 的点击改指 `heading2`，spec 退出 **1**，还原后退出 **0**。行格式没覆盖的三个字段（`enabled`、`checked`、分隔条自己的 `visible`）在同一文件里另加一例直接断言。
...
| PR-8      | O7② 读通道路径域 —— **实现到划下来的那条线** `security/picker-grants-and-read-scope`：选择器结果即授权、`mt::uploader::upload` 载荷归属（实测是"渲染端可指定主进程执行哪个程序"的信任边界洞，已闭）、`read-file`/`readdir`/`rg::start` 的目录收域 + 6 条单测与 1 条 E2E 负例 | PR-7。**两条尾巴已收尾**：`mt::rg::start` 的载荷归属做完于 `8c0d917`（O8 仅剩的那处豁免随之消失），五个布尔探测通道**决定不收域**（它们只回答"是不是 / 在不在"，收域换不到读不到的东西，理由写在 `fs.ts` 注释里）。O7② 到此封口 |
...
| PR     | 内容                                                                                                                                                                                                                                                                                  | 说明                                                                                                                                                                        |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PR-13… | O12 上帝文件分解（store 提 services → 拆 composable，每步 E2E 全绿）—— **第 1-6 步已实现**（第 6 步 `2a808d8`+`1714dcf` 起动 editor.vue）（前四步 `0839766`/`88274b5`/`ed03def`/`83eb532`，第 5 步六个提交；store 2347 → 1977 行、editor.vue 2321 → 2169 行，注册型动作已不含业务体） | 每步可独立 revert，禁止与功能改动混提；下一刀见 §3 O12 "第 4 步落点"                                                                                                        |
...
| desktop E2E `pnpm test:e2e`（Windows 本机，真窗口） | 预演首轮 222 通过 / **5 失败**；修掉自己那条后 223 / **4 失败**；逐个合入后的 `develop`（与预演树逐字节相同）222 / **5 失败** —— 多出的那条是 `tab-switch-cursor.spec.ts`，单独连跑 3 次全绿，属全量并发下的偶发超时；**2026-09-21 修掉测试自身的换行假设后（`a230857`）为 226 通过 / 1 失败**，唯一那条是另一处偶发（`toc-panel-content:152`，全量里跑到 8.4s 才失败，单跑两次 4/4 绿、各 3.2s） |
...
- **上面那 4 条已根因并修掉（`a230857`，分支 `test/roundtrip-eol-baseline`）**：不是内容不稳定，也不是引擎在 Windows 上改写文档。spec 里 `readDisk()` 原样读取仓库 fixture 当"期望值"，而这个 checkout 因 `core.autocrlf` 是 CRLF（`git ls-files --eol` 给出 `i/lf w/crlf attr/`，且全仓**没有** `.gitattributes`），而引擎序列化是 LF —— 于是每一行都差一个 `\r`（Playwright 的 diff 把它印成行尾空格，所以看起来"只有 front matter 行有空格差异"），Linux 上恰好两侧都是 LF 才对得上。修法是断言比较用**入库形态**（新增 `asCommitted()` 只做 CRLF→LF），**teardown 仍按原始字节还原文件**，所以不会把任何人的工作副本改形状；没动 git 配置、也没新增 `.gitattributes`（那会让全仓检出行为一起变，不在这次范围里）。
