...
- 这一刀的门禁：`pnpm check` **0**（**99 warnings / 0 errors**，未新增）、`pnpm test:unit` **0**（82 文件 / 1001 通过 + 1 跳过，+5 是形状锁那一份）、`pnpm build` **0**、`pnpm knip` **0**；全量 `pnpm test:e2e` **233 通过 / 4 跳过 / 1 失败（退出 1，9.1 分钟）**——失败点是 `test/e2e/crash-range-offset.spec.ts:33` 的 `electron.launch` 30 秒超时，**不是断言**；把该文件单独重跑，其余 8 条全绿、同一条仍红在同一处，签名与上面记的启动层偶发一致（`0056c62` 那条曾对 `develop` 跑过对照，同类位置同样会红）。
