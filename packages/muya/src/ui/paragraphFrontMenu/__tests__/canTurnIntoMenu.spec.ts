...
// Regression for colamd commit 7b7a9424 "should not nest math block into
// other math block (#1153)".
//
// In colamd, `insertContainerBlock(functionType, block)` walked from the
...
// nested inside a math-block. The colamd fix added `getAnchor(block)` +
...
describe('canTurnIntoMenu — no nesting math/code/html/diagram inside themselves (colamd 7b7a9424)', () => {
...
// Regression for colamd commit f00da152 (#812 — "insert table into `table`,
...
// crashed on re-parse. colamd's fix added `getAnchor(block)` + "abort if
// no anchor" gating.
//
...
describe('canTurnIntoMenu — no inserting tables inside non-paragraph containers (colamd f00da152)', () => {
