...
// legacy colamd inline lexer used to fail before commit 57cd04c5 (Apr 2019,
// colamd PR #957). The new muya's inline lexer (`validateEmphasize` +
...
// the full markdown→HTML pipeline, because the inline lexer is what colamd
...
describe('inline lexer — CommonMark 0.29 spec examples (colamd 57cd04c5)', () => {
...
// Defensive regression for colamd commit d9f64bab (issue #921, PR #947):
...
describe('inline lexer — reference link (colamd d9f64bab)', () => {
...
// Defensive regression for colamd commit 8e32838b (PR #1531) — sup/sub
// inline syntax `^foo^` (superscript) and `~bar~` (subscript). Already
// present in the new muya's inline lexer rules + renderer.
describe('inline lexer — superscript/subscript (colamd 8e32838b)', () => {
...
// Defensive regression for colamd commit c0853f64 (PR #1421):
...
// colamd fix introduced (`top && (pos === 0 || /[* _~(]{1}/...))`).
describe('inline lexer — auto link (colamd c0853f64)', () => {
...
// Defensive regression for colamd commit ad5ddbf9 (GFM example 558, PR #917):
...
describe('inline lexer — GFM link/image title (colamd ad5ddbf9)', () => {
...
// Defensive regression for colamd commit d937fac0 (issue #1071, PR #1072):
...
describe('inline lexer — repeated bold + inline_code (colamd d937fac0 / #1071)', () => {
...
// Defensive regression for colamd commit 57af8304 (issue #1169, PR #1170):
...
// same algorithm.
describe('inline lexer — link / image dest with parens (colamd 57af8304 / #1169)', () => {
    ...
    it('stops at the FIRST matching `)` when more `)` appear later on the line', () => {
        // This is the real shape of the colamd regression: greedy `(.*)` in
...
// Regression for colamd #3778. `lowerPriority` scanned every position for a
