...
const cli = (): ParsedArgs => {
  ...
  if (process.env.NODE_ENV === 'development') {
    // Don't pass electron development arguments to ColaMD and change user data path.
    argv = ['--user-data-dir', path.join(getPath('appData'), 'colamd-dev')]
  }

  const args = parseArgs(argv, true)
  if (args['--help']) {
    write(`Usage: colamd [commands] [path ...]
...
`)
  ...
  if (args['--version']) {
    writeLine(`ColaMD: ${COLAMD_VERSION_STRING}`)
  ...
  if (!args['--user-data-dir']) {
    const portablePath = path.join(app.getAppPath(), '..', '..', 'colamd-user-data')
