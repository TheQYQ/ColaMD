...
// `window.colamd.paths` / `window.fileUtils` at call time, all normally
...
vi.hoisted(() => {
  const w = globalThis as unknown as {
    window?: {
      ...
      colamd?: { paths: { userDataPath: string } }
      fileUtils?: {
...
// never touch `window.fileUtils`/`window.colamd`, unlike a disk theme name)
